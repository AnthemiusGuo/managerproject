<div>
    <table class="table table-striped simplePagerContainer">
        <thead>
            <tr>
                <?
                foreach ($this->badBookList->build_inline_list_titles() as $key_names):
                ?>
                    <th>
                        <?php
                        echo $this->badBookList->dataModel[$key_names]->gen_show_name();;
                        ?>
                    </th>
                <?
                endforeach;
        ...
        <tbody class="crm-book-table-paged" id="crm-book-table">
            ...
            foreach($this->badBookList->record_list as  $this_record): ?>
                <tr>
                    <?
                    foreach ($this->badBookList->build_inline_list_titles() as $key_names):
                    ?>
                    ...
                    <td>
                        <?
                        if ($this->badBookList->is_lightbox):
    ...
    <?
    if (count($this->badBookList->record_list)<=0){
    ?>
